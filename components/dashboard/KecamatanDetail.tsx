...
type KecamatanDetailProps = {
  kecamatan: KecamatanCardItem;
  partyList?: KecamatanPartyCardItem[] | null;
  topCandidateList?: KecamatanTopCandidateCardItem[] | null;
  isLoading: boolean;
  errorMessage: string;
  openedPartyId: number | null;
  onBack: () => void;
  onOpenPartyDetail: (partai: KecamatanPartyCardItem) => void;
  renderPartyDetail?: (partai: KecamatanPartyCardItem) => ReactNode;
};

/**
 * Detail kecamatan berisi grafik partai, top kandidat, dan rekap partai.
 */
export default function KecamatanDetail({
  ...
  onOpenPartyDetail,
  renderPartyDetail,
}: KecamatanDetailProps) {
  const safePartyList = Array.isArray(partyList) ? partyList : [];
  const safeTopCandidateList = Array.isArray(topCandidateList)
    ? topCandidateList
    : [];

  const totalSuaraKecamatan =
    safePartyList[0]?.totalSuaraKecamatan ?? kecamatan.totalSuara;

  const maxSuaraPartai = Math.max(
    1,
    ...safePartyList.map((partai) => partai.totalSuara)
  );
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Grafik dan rekap suara pada kecamatan yang dipilih.
          </p>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-4">
          ...
          <div className="rounded-3xl bg-slate-50 px-5 py-4">
            ...
            <p className="mt-2 text-3xl font-black text-slate-950">
              {formatNumber(totalSuaraKecamatan)}
            </p>
          </div>
          ...
          <div className="rounded-3xl bg-blue-50 px-5 py-4">
            ...
            <p className="mt-2 text-sm font-black leading-6 text-blue-700">
              {formatNumber(kecamatan.totalKelurahan)} kelurahan ·{" "}
              {formatNumber(kecamatan.totalTps)} TPS
            </p>
          </div>
        </div>
      </section>
        ...
        <>
          <section className="w-full max-w-full overflow-hidden rounded-4xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Grafik ini hanya menampilkan suara partai pada Kecamatan{" "}
                  {kecamatan.nama}.
                </p>
              </div>
            ...
            {safePartyList.length > 0 ? (
              <>
                <div className="space-y-3 lg:hidden">
                  {safePartyList.map((partai) => {
                    ...
                    const percent =
                      totalSuaraKecamatan > 0
                        ? (partai.totalSuara / totalSuaraKecamatan) * 100
                        : 0;
                    ...
                    return (
                      <article
                        key={`mobile-chart-${partai.partaiId}`}
                  ...
                  <div className="w-full max-w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-6">
                    <div
                      ...
                      style={{
                        ...
                        gridTemplateColumns: `repeat(${safePartyList.length}, minmax(0, 1fr))`,
                      }}
                    >
                      {safePartyList.map((partai) => {
                        const color = getPartyColor({
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                <p className="text-sm font-semibold text-slate-600">
                  Data grafik kecamatan belum tersedia.
                </p>
              </div>
            )}
          </section>
          ...
          <section className="rounded-4xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="mb-5">
              ...
              <h3 className="mt-2 text-xl font-black text-slate-950 sm:text-2xl">
                Top 5 Kandidat Se-Kecamatan
              </h3>

              <p className="mt-2 text-sm leading-6 text-slate-600">
                Kandidat perorangan dengan suara tertinggi di Kecamatan{" "}
                {kecamatan.nama}.
              </p>
            </div>
            ...
            {safeTopCandidateList.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="mt-4 text-center">
                      ...
                      <div className="mt-3 rounded-2xl bg-white px-3 py-3">
                        ...
                        <p className="mt-1 text-lg font-black text-slate-950">
                          {formatNumber(candidate.totalSuaraKecamatan)}
                        </p>
                      </div>
                    </div>
                  </article>
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                <p className="text-sm font-semibold text-slate-600">
                  Data top kandidat kecamatan belum tersedia.
                </p>
              </div>
            )}
          </section>
          ...
          <section className="rounded-4xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="mb-5 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
              <div>
                <h3 className="text-xl font-black text-slate-950">
                  Rekap Partai Per Kecamatan
                </h3>

                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Klik Detail Partai untuk membuka suara kandidat per
                  kelurahan.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3 lg:hidden">
              {safePartyList.length > 0 ? (
                safePartyList.map((partai) => {
                  ...
                  return (
                    <article
                      key={`mobile-party-recap-${partai.partaiId}`}
                      className="rounded-3xl border border-slate-200 bg-slate-50 p-4"
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <button
                            type="button"
                            onClick={() => onOpenPartyDetail(partai)}
                            className={
                              isOpened
                                ? "mt-3 w-full rounded-2xl bg-slate-950 px-4 py-2.5 text-sm font-black text-white shadow-sm transition"
                                : "mt-3 w-full rounded-2xl bg-red-600 px-4 py-2.5 text-sm font-black text-white shadow-sm transition hover:bg-red-700"
                            }
                          >
                            {isOpened ? "Dibuka" : "Detail Partai"}
                          </button>
                        </div>
                      </div>

                      {isOpened && renderPartyDetail ? (
                        <div className="mt-4">{renderPartyDetail(partai)}</div>
                      ) : null}
                    </article>
                  );
                })
                ...
                <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                  <p className="text-sm font-semibold text-slate-600">
                    Data rekap partai kecamatan belum tersedia.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="hidden overflow-x-auto rounded-3xl border border-slate-200 lg:block">
              <table className="min-w-full border-collapse bg-white">
                ...
                <tbody>
                  {safePartyList.map((partai, index) => {
                    const isOpened = openedPartyId === partai.partaiId;

                    return (
                      <Fragment key={`party-row-group-${partai.partaiId}`}>
                        <tr
                          className={
                        ...
                        >
                          ...
                          <td className="border-b border-slate-100 px-4 py-3 text-center">
                            <button
                              ...
                              className={
                                isOpened
                                  ? "rounded-2xl bg-slate-950 px-4 py-2 text-xs font-black text-white shadow-sm transition"
